...
public class RestApiUtil {
     ...
     *
     */
    public static void delDataSource(TableauCredentialsType credential, String projectName, String dataSource){
        TsResponse tsResponse = showDataSource(credential);
        DataSourceListType datasources = tsResponse.getDatasources();
        List<DatasourceType> dataSourceArr = datasources.getDataSource();
        DatasourceType datasourceType = null;
        if(dataSourceArr!=null){
            Stream<DatasourceType> datasourceTypeStream = dataSourceArr.stream().filter(eo -> projectName.equals(eo.getProject().getName()) && dataSource.equals(eo.getName()));
            List<DatasourceType> collect = datasourceTypeStream.collect(Collectors.toList());
            if(collect.size()>0){
                datasourceType= collect.get(0);
            }

        }
        if(datasourceType!=null){
            String url = Operation.DEl_DATA_SOURCE.getUrl( credential.getSite().getId(),datasourceType.getId());
            httpConnection(url,"delete",credential.getToken());
        }
    }
